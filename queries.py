...
def Q_1(conn, execution_time):
    ...
    query = """ 
        SELECT p.player_name, AVG(s.statsbomb_xg) AS avg_xgScore 
        FROM players p
...
def Q_2(conn, execution_time):
    ...
    query = """ 
        SELECT p.player_name AS player_name, COUNT(e.id) AS num_shots
        FROM events e
...
def Q_3(conn, execution_time):
    ...
    query = """
        SELECT p.player_name, COUNT(e.id) AS num_first_shot
        FROM events e
        JOIN players p ON p.player_id = e.player_id AND e.match_id = p.match_id
        JOIN matches m ON m.match_id = e.match_id
...
def Q_4(conn, execution_time):
    ...
    query = """
        SELECT t.team_name, COUNT(e.id) AS num_passes
        FROM teams t
...
def Q_6(conn, execution_time):
    ...
    query = """
        SELECT t.team_name, COUNT(e.id) as num_shots
        FROM teams t
        JOIN events e ON e.team_id = t.team_id
        JOIN matches m ON m.match_id = e.match_id
        JOIN competitions c ON c.competition_id = m.competition_id AND m.season_id = c.season_id
...
def Q_7(conn, execution_time):
    ...
    query = """ 
        SELECT p.player_name, COUNT(e.id) num_through_balls
        FROM events e
        JOIN players p ON e.player_id = p.player_id AND e.match_id = p.match_id
        JOIN matches m ON m.match_id = e.match_id
        JOIN competitions c ON c.competition_id = m.competition_id AND m.season_id = c.season_id
...
def Q_8(conn, execution_time):
    ...
    query = """ 
        SELECT t.team_name, COUNT(e.id) AS num_through_balls
        FROM teams t
        JOIN events e ON e.team_id = t.team_id
        JOIN matches m ON m.match_id = e.match_id
        JOIN competitions c ON c.competition_id = m.competition_id AND m.season_id = c.season_id
...
def Q_10(conn, execution_time):
    ...
    query = """ 
        SELECT p.player_name, COUNT(e.id) AS num_dribbled_past
        FROM events e
        JOIN players p ON e.player_id = p.player_id AND e.match_id = p.match_id
        JOIN matches m ON m.match_id = e.match_id
        JOIN competitions c ON c.competition_id = m.competition_id AND m.season_id = c.season_id
